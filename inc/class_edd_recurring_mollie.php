    ...
    public function create_payment_profiles() {
        ...
        if ( empty( $customer->currency ) || $customer->currency != strtolower( $currency ) ) {
            ...
            try {
                ...
                /*
                 * Currency code alreay set on customer. $customer->currency is the only one that may be used for this customer.
                 *
        ...
        foreach( $this->subscriptions as $key => $subscription ) {
            ...
            $tax_percentage = 0;
            $adjust_for_tax = false;

            if ( ! empty( $subscription['initial_tax'] ) && ! edd_prices_include_tax() ) {

                edd_debug_log( 'Subscription includes tax, calculating' );
            ...
            if( $adjust_for_tax ) {
                // We now have to add taxes back into the recurring and initial amounts so that the balance adjustment comes out correctly
                $subscription['initial_amount']   += $subscription['initial_tax'];
                $subscription['recurring_amount'] += $subscription['recurring_tax'];
            }
    ...
    public function update_payment_method( $subscriber, $subscription ) {
        ...
        if ( empty( $errors ) ) {
            ...
            try {
                ...
                if ( isset( $_POST['edd_stripe_token'] ) ) {
                    $card = $cu->sources->create( array( "source" => $_POST['edd_stripe_token'] ) );
